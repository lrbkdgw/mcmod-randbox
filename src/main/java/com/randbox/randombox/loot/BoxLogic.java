...
public final class BoxLogic {
 ...
 @SubscribeEvent public static void open(PlayerInteractEvent.RightClickBlock e){
  if(e.getHand()!=InteractionHand.MAIN_HAND||e.getLevel().isClientSide()||!(e.getEntity() instanceof ServerPlayer player)||!(e.getLevel().getBlockEntity(e.getPos()) instanceof ChestBlockEntity chest)||lootTable(chest)==null)return;
  e.setCanceled(true);
  if(chest.getPersistentData().getBoolean(DRAWING))return;
  ServerLevel level=(ServerLevel)e.getLevel();ResourceLocation tableId=lootTable(chest);BoxTier tier=chest.getPersistentData().contains(TIER)?BoxTier.byId(chest.getPersistentData().getInt(TIER)):BoxTier.roll(level.random);
 ...
 private static ResourceLocation lootTable(ChestBlockEntity chest){String id=chest.saveWithFullMetadata().getString("LootTable");return id.isEmpty()?null:ResourceLocation.tryParse(id);}
