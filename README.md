...
> **跳过 / 取消规则**
> * 生存模式：动画不可跳过，但按 **ESC 可以取消本次抽奖** —— 不发奖、箱子保持未开启状态，
>   下次打开会重新抽一次（`CancelReelPacket` → `ReelManager#cancel`）。
> * 创造模式：按 **空格直接跳过**动画，立即拿到已经定好的奖品（界面底部会提示）。
> * 除此之外的任何按键、鼠标点击都无效；若界面被其它途径关闭（服务端换屏、第三方 mod），
>   `removed()` 与客户端每 tick 的守卫会把它重新放回最前面。
> * 客户端断线时服务端超时后照常发奖；同一玩家重新打开同一个箱子会**重播同一次抽奖**，不会重抽。
...
* 服务端每秒把玩家周围（默认 32 格；若装备「宝藏感知」会自动扩到光柱所需距离）的箱子数据同步给客户端。
* 客户端为每个箱子按稀有度颜色（白/绿/紫/黄/红）在四周环绕 `dust` 粒子。
* **未开启过**的箱子在靠近时（默认 12 格，旧默认 24 格的 50%）额外渲染一道信标式光柱（`BeaconRenderer.renderBeaconBeam`）。
* **未开箱就挖掉**：与原版一致，不掉落任何战利品（未抽取的战利品表直接作废），
  只是会把该位置从记录里删掉，保证光柱 / 粒子立即消失。
...
* **宝藏感知**（头盔，稀有度等同于时运，最高 3 级，非宝藏型）：光柱可见距离变为基础值的 150% / 200% / 250%。
* **快速寻宝**（靴子，稀有度等同于保护，最高 4 级，非宝藏型）：抽奖开箱时间变为原来的 80% / 60% / 40% / 20%。
* **宝藏深掘**（裤子，最高 2 级，宝藏型）：穿戴后开箱奖励物品数 +1 / +2；仅会作为 1 级附魔书出现在远古城市战利品里，权重跟随附魔金苹果并按 1/3 系数、Quality 3 注入。
* **宝藏升华**（胸甲，最高 1 级，宝藏型）：穿戴后有 15% 概率把非绝世宝箱的品质升一级；仅会作为 1 级附魔书出现在末地城/末地船宝藏表里，权重跟随钻石并按 1/15 系数、Quality 3 注入。该 15% 判定会在第一次对该宝箱生效时立刻写入宝箱数据，因此即使按 ESC 取消后重开，也不会重新判定。
* 抽奖读取原版 / 数据包战利品表时会使用原版的 loot condition 反序列化与判定（例如随机概率、实体条件等），再保留原版 loot function 生成附魔书 / 随机数量 / 附魔装备，避免破坏原版兼容性。

### 监守者触角

* **监守者触角**：击杀监守者时有 10% 概率掉落，每级抢夺额外 +2%。物品耐久 6、不可堆叠。
* 手持监守者触角右键一个仍带 `LootTable`、尚未开启过的战利品箱，会消耗 1 点耐久并提前查看当前抽奖结果。
* 预览会把看到的奖品写入宝箱数据；之后正常开箱、ESC 取消后再开，都会使用这份已固定的结果。
* 预览界面可以选择「保留结果」或「重抽结果」。每次使用触角只授权一次重抽；重抽后的新结果会保存到该宝箱。
